...
class BookingAdmin(admin.ModelAdmin):
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        # -------------------------
        # ADD FORM (obj is None)
        # -------------------------
        if obj is None:
            # All fields should be editable when adding a booking
            return base   # Only timestamps are readonly

        # -------------------------
        # CHANGE FORM (edit page)
        # Only booking_status editable
        # -------------------------
        return base + ['user', 'event', 'slot', 'attendees_count']
